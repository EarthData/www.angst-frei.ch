var margin = {top: 30, right: 70, bottom: 40, left: 40},
  width = parseInt(d3.select('#covid').style('width'), 10),
...
const graph = async (group, ydomain) => {
  ...
  if (group == "covid") {
    var title = "Hospitalisationen nach Alter (Covid)";
  } else {
    var title = "Hospitalisationen nach Alter (Covid-geimpft)";
  }
  ...
  d3.csv("data_hosp_processed/hosp_" + group + ".csv")
    .then(function(data) {
      ...
      function restorePlot(d) {
        ...
        for (i = 0; i < legendClassArray[group].length; i++) {
          if (legendClassArray[group][i] != class_keep) {
            d3.selectAll(".class-" + group + "-" + legendClassArray[group][i])
              .transition()
              .duration(1000)
              .delay(750)
              .style("opacity", 1);
