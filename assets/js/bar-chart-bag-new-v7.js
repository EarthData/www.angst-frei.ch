var margin = {top: 30, right: 70, bottom: 40, left: 40},
  width = parseInt(d3.select('#ch-old').style('width'), 10),
...
var divs = ["ch-old", "ch-young", "region-old", "region-young"]
...
d3.select("#selectYearButton")
  ...
  .text(function (d) { return d; }) // text showed in the menu
  .attr("value", function (d) { return d; }) // corresponding value returned by the button
  .property("selected", "2020")

const graph = async (year, region, type) => {

  var group = region == "first" ? "ch-" + type : "region-" + type;
  if (region == "first") {
    region = "CH"
  }
...
const build = async () => {
  await graph(2021, "first", "old");
  await graph(2021, "first", "young");
  await graph(2020, "CH", "old");
  await graph(2020, "CH", "young");
}

build();

// When the button is changed, run the updateChart function
d3.select("#selectRegionButton").on("change", function(d) {
  var selectedRegion = d3.select(this).property("value")
  var selectedYear = d3.select("#selectYearButton").property("value")
  changeDrop(selectedRegion, selectedYear)
})

d3.select("#selectYearButton").on("change", function(d) {
  var selectedYear = d3.select(this).property("value")
  var selectedRegion = d3.select("#selectRegionButton").property("value")
  changeDrop(selectedRegion, selectedYear)
})

const changeDrop = async(region, year) => {
  await graph(year, region, "old");
  await graph(year, region, "young");
}
