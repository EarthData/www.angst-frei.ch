...
const graph = async (year, region, group, ydomain) => {
  ...
  d3.csv("data_bag_processed/death_" + year + "_" + region + "_" + group + ".csv")
    .then(function(data) {
...
const build = async (year) => {
  await graph(year, "CH", "old", 2200);
  await graph(year, "CH", "young", 200);
}
