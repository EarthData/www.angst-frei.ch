...
const cum_data = d3.text("death.csv").then(function(raw) {
  ...
  data.forEach(function(d) {
    ...
    var absolute_date = parseWeek(d['TIME_PERIOD']);
    var week          = week_formater(new Date(absolute_date));
    var year          = year_formater(new Date(absolute_date));
    d['TIME_PERIOD']  = week;
    d['YEAR']         = year;
    d['GEO']          = geolong[d['GEO']];
  });
  ...
  for (var i = 0; i < data.length; i++) {
    if (data[i]['AGE'] != "_T") {
      death_data[data[i]['YEAR']][data[i]['GEO']][data[i]['TIME_PERIOD']][data[i]['AGE']][data[i]['SEX']] = parseInt(data[i]['Obs_value'])
    }
  }
...
cum_data.then(async function(cum_data) {
  ...
  d3.text("corona.csv").then(function(raw) {
    var dsv = d3.dsvFormat(',');
    var data = dsv.parse(raw);
    ...
    corona_data['TIME_PERIOD'] = d3.map(data, function(d){return d['TIME_PERIOD']}).keys()
    ...
    corona_data['YEAR'].map((value0, index0) => {
      ...
      corona_data['GEO'].map((value1, index1) => {
        ...
        corona_data['TIME_PERIOD'].map((value2, index2) => {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i]['ncumul_deceased'] != "" && data[i]['ncumul_deceased'] > corona_data[data[i]['YEAR']][data[i]['abbreviation_canton_and_fl']][data[i]['TIME_PERIOD']]) {
        corona_data[data[i]['YEAR']][data[i]['abbreviation_canton_and_fl']][data[i]['TIME_PERIOD']] = parseInt(data[i]['ncumul_deceased'])
      }
    }
  });
  ...
  for (var geo of death_data['GEO']) {
    ...
    for (var week in death_data[geo]) {
      var values = {}
      var lastweek = week-1;
      values['TIME_PERIOD'] =  week;
      if (region_data_keys.indexOf(week) === -1) {
        region_data_keys.push(week);
      }
      var total = 0
  ...
  const keys = ['Y0T4', 'Y5T9', 'Y10T14', 'Y15T19', 'Y20T24', 'Y25T29', 'Y30T34', 'Y35T39', 'Y40T44', 'Y45T49', 'Y50T54', 'Y55T59', 'Y60T64', 'Y65T69', 'Y70T74', 'Y75T79', 'Y80T84', 'Y85T89', 'Y_GE90']
